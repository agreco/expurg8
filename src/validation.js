...
// public properties
		...
		min     : 0,
// public methods
		coerce  : function( v ) { return this.prune( ( is_arr( v ) ? v : [v] ).map( this.type.coerce, this.type ) ); },
		valid   : function( v ) {
			return this.hasMany
				? is_arr( v ) && v.length >= this.min && v.length <= this.max
				: !( this.type instanceof __lib__.type.Array ) && !is_arr( v );
		},
// internal methods
	...
	// if the property is multiple & required     -> min = M, max = N
		init    : function()    { this.hasMany = this.min > 1 && this.max > this.min; },
		prune        : function( v ) {
